...
describe("scripts/resolve-mentions.ts", () => {
  ...
  it("resolves user mentions inside JSON day-bucket object cleanly", async () => {
    ...
    expect(parsed.entries[0].parent.text).toBe("Hello @Robert Bell and @Em Fazza!");
    expect(parsed.entries[0].replies[0].text).toBe("Thanks @Robert Bell.");
  ...
  it("gracefully falls back to plain-text replacement if input is not JSON", async () => {
    ...
    expect(stdout.trim()).toBe("Message with @Robert Bell and <@U00000>.");
  });
});
