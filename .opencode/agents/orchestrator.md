...
4. Delegated PR Review & UAT Pipeline: Immediately when a worker opens a Pull Request, the Orchestrator delegates PR review and UAT verification to the `qa-validator` subagent using `DelegateTask`. This isolates raw test execution logs, diffs, and tracebacks within the subagent's ephemeral context, ensuring only the final PASS/FAIL summary returns to the Orchestrator.
